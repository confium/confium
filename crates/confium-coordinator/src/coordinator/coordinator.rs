...
/// Trait for threshold signing engines. The coordinator delegates
/// the actual cryptographic aggregation to whatever signer is
/// injected, avoiding circular dependencies between `confium-tc`
/// and the scheme-specific crates (`confium-tc-cmp20`, etc.).
pub trait ThresholdSigner: Send + Sync {
    /// Produce a threshold signature from T shares.
    fn sign(
        &self,
        scheme: &str,
        shares: &[Vec<u8>],
...
/// Default mock signer — XORs all share bytes together.
...
impl ThresholdSigner for MockSigner {
    fn sign(
        ...
        for share in shares {
            for (i, b) in share.iter().take(64).enumerate() {
                aggregated[i] ^= b;
            }
        }
        Ok(aggregated)
...
impl Coordinator {
    /// Construct a new coordinator with the default mock signer.
    pub fn new() -> Self {
        Self {
    ...
    /// Construct a coordinator with a custom threshold signer.
    /// Production callers inject a real CMP20/GG18 signer here.
    pub fn with_signer(signer: Box<dyn ThresholdSigner>) -> Self {
        Self {
            sessions: HashMap::new(),
            audit_log: AuditLog::new(),
            next_id: 0,
    ...
    #[tracing::instrument(skip(self), fields(quorum = %request.quorum_id, scheme = %request.scheme, threshold = request.threshold))]
    ...
    #[tracing::instrument(skip(self, commitment), fields(session = %session_id, signer = %commitment.signer_id))]
    ...
    #[tracing::instrument(skip(self, share), fields(session = %session_id, signer = %share.signer_id))]
    ...
    ///
    /// Calls the injected `ThresholdSigner` to produce a real
    /// threshold signature. The default signer is `MockSigner`
    /// (XOR-based); production callers inject a real signer via
    /// [`Coordinator::with_signer`].
    #[tracing::instrument(skip(self), fields(session = %session_id))]
    pub fn aggregate(&mut self, session_id: &str) -> Result<AggregatedSignature, SessionError> {
        ...
        let aggregated = self
            .signer
            .sign(&scheme, &share_blobs, threshold, &message)
    ...
    /// Mutable reference to audit log.
    pub fn audit_log_mut(&mut self) -> &mut AuditLog {
        &mut self.audit_log
    }

    /// All session IDs currently in the coordinator.
    pub fn session_ids(&self) -> Vec<SessionId> {
        self.sessions.keys().cloned().collect()
    }

    /// Mutable reference to a specific session.
    pub fn session_mut(&mut self, session_id: &str) -> Option<&mut CoordinatorSession> {
        self.sessions.get_mut(session_id)
    }

    /// Directly set a session's state (used by the reaper and admin actions).
    pub fn set_session_state(&mut self, session_id: &str, state: SessionState) {
