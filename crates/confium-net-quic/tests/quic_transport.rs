...
//!
//! These exercise the real QUIC path: bind on `127.0.0.1:0` (OS-assigned
//! ephemeral port to avoid CI conflicts), connect, exchange framed
//! messages both directions, and confirm clean close terminates both
//! sides. They go through the high-level [`confium_net::connect`] /
//! [`confium_net::listen`] entry points so the link-time registry
//! dispatch is exercised end-to-end.
//!
//! Depending on `confium-net-quic` from the test binary is what links
//! the crate (and its `register_transport!` submission) into the test
//! binary — without it, the `quic` scheme would be unknown to the
//! registry.
...
fn round_trip_single_message_via_registry() {
    ...
    let listener =
        confium_net_quic::QuicListener::bind("quic", "127.0.0.1", 0).expect("bind failed");
    let port = listener.local_addr().expect("local_addr").port();
    let mut listener: Box<dyn net::Listener> = Box::new(listener);
...
fn multiple_messages_preserve_order_and_boundaries() {
    ...
    // three separate recvs, not one coalesced byte blob.
    let listener =
        confium_net_quic::QuicListener::bind("quic", "127.0.0.1", 0).expect("bind failed");
    let port = listener.local_addr().expect("local_addr").port();
    let mut listener: Box<dyn net::Listener> = Box::new(listener);
...
fn missing_port_is_rejected() {
    // `quic://host` with no port must be rejected.
    let result = net::connect("quic://127.0.0.1");
    let err = match result {
        Ok(_) => panic!("expected MalformedUrl error, got Ok"),
        Err(e) => e,
    };
    assert!(
        matches!(err, net::Error::MalformedUrl { ref scheme, .. } if scheme == "quic"),
        "expected MalformedUrl, got {err:?}"
    );
}

#[test]
fn quic4_loopback_round_trip() {
    // The `quic4` scheme restricts to IPv4. Loopback is IPv4.
    let listener =
        confium_net_quic::QuicListener::bind("quic4", "127.0.0.1", 0).expect("bind failed");
    let port = listener.local_addr().expect("local_addr").port();
    let mut listener: Box<dyn net::Listener> = Box::new(listener);
    let url = format!("quic4://127.0.0.1:{port}");

    let url_clone = url.clone();
    let client_handle = thread::spawn(move || -> net::Result<()> {
        let mut client = net::connect(&url_clone)?;
        client.send(b"quic4 hello")?;
        client.close()?;
        Ok(())
    });
    ...
    let mut buf = [0u8; 32];
    let n = server.recv(&mut buf).expect("recv");
    assert_eq!(&buf[..n], b"quic4 hello");
    server.close().expect("server close");
    client_handle.join().unwrap().unwrap();
}

#[test]
fn quic6_loopback_round_trip() {
    // The `quic6` scheme restricts to IPv6. Loopback is `[::1]`.
    let listener = confium_net_quic::QuicListener::bind("quic6", "::1", 0).expect("bind failed");
    let port = listener.local_addr().expect("local_addr").port();
    let mut listener: Box<dyn net::Listener> = Box::new(listener);
    let url = format!("quic6://[::1]:{port}");

    let url_clone = url.clone();
    let client_handle = thread::spawn(move || -> net::Result<()> {
        let mut client = net::connect(&url_clone)?;
        client.send(b"quic6 hello")?;
        client.close()?;
        Ok(())
    });
