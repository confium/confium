...
impl TraceContext {
    ...
    pub fn from_w3c_header(header: &str) -> Option<Self> {
        ...
        Some(Self {
            trace_id: parts[1].into(), span_id: parts[2].into(),
            parent_span_id: None, baggage: HashMap::new(),
        })
    }
...
impl LogEntry {
    pub fn info(msg: &str) -> Self {
        Self { timestamp: Utc::now(), level: "info".into(), message: msg.into(),
               fields: HashMap::new(), trace_id: None }
    }

    pub fn error(msg: &str) -> Self {
        Self { timestamp: Utc::now(), level: "error".into(), message: msg.into(),
               fields: HashMap::new(), trace_id: None }
    }

    pub fn warn(msg: &str) -> Self {
        Self { timestamp: Utc::now(), level: "warn".into(), message: msg.into(),
               fields: HashMap::new(), trace_id: None }
    }
...
impl StructuredLogger {
    pub fn new(min_level: &str) -> Self {
        Self { entries: Mutex::new(Vec::new()), min_level: Mutex::new(min_level.into()) }
    }

    pub fn log(&self, entry: LogEntry) {
        let levels = ["trace", "debug", "info", "warn", "error"];
        let min_idx = levels.iter().position(|&l| l == &*self.min_level.lock().unwrap()).unwrap_or(2);
        let entry_idx = levels.iter().position(|&l| l == entry.level.as_str()).unwrap_or(2);
        if entry_idx >= min_idx {
            self.entries.lock().unwrap().push(entry);
        }
    }

    pub fn entries(&self) -> Vec<LogEntry> { self.entries.lock().unwrap().clone() }
    pub fn count(&self) -> usize { self.entries.lock().unwrap().len() }
    pub fn to_jsonl(&self) -> String {
        self.entries.lock().unwrap().iter().map(|e| e.to_jsonl()).collect()
    }
...
impl CardinalityLimiter {
    pub fn new(max_values_per_label: usize, max_total_series: u64) -> Self {
        Self {
            ...
            max_values_per_label, total_series: AtomicU64::new(0), max_total_series,
        }
    }

    pub fn allow_label(&self, label_name: &str, label_value: &str) -> bool {
        if self.total_series.load(Ordering::SeqCst) >= self.max_total_series { return false; }
        let mut counts = self.label_counts.lock().unwrap();
        let values = counts.entry(label_name.into()).or_default();
        if values.contains(label_value) { return true; }
        if values.len() >= self.max_values_per_label { return false; }
    ...
    pub fn label_value_count(&self, label: &str) -> usize {
        self.label_counts.lock().unwrap().get(label).map(|s| s.len()).unwrap_or(0)
    }

    pub fn total_series(&self) -> u64 { self.total_series.load(Ordering::SeqCst) }
    pub fn reset(&self) {
...
pub fn runs_test(data: &[u8]) -> f64 {
    let bits: Vec<i8> = data.iter().flat_map(|b| (0..8).map(move |i| ((b >> i) & 1) as i8)).collect();
    let n = bits.len() as f64;
    let pi: f64 = bits.iter().map(|&b| b as f64).sum::<f64>() / n;
    if (pi - 0.5).abs() >= 2.0 / n.sqrt() { return 0.0; }
    let mut v_obs = 1.0;
    for i in 1..bits.len() {
        if bits[i] != bits[i-1] { v_obs += 1.0; }
    }
...
fn erfc(x: f64) -> f64 {
    ...
    let z = x.abs();
    let t = 1.0 / (1.0 + 0.5 * z);
    let r = t * (-z * z - 1.26551223 + t * (1.00002368 + t * (-0.37609177 +
        t * (0.36496351 + t * (-0.16108111 + t * (0.08095456 + t * (-0.02907307)))))))
        .exp();
...
impl ZeroizationAuditor {
    pub fn new() -> Self { Self { tracked_secrets: Mutex::new(Vec::new()) } }

    pub fn track(&self, id: &str, size: usize) {
        self.tracked_secrets.lock().unwrap().push(SecretRecord {
            id: id.into(), allocated_at: Utc::now(), zeroized: false, size_bytes: size,
        });
    }

    pub fn mark_zeroized(&self, id: &str) {
        if let Some(s) = self.tracked_secrets.lock().unwrap().iter_mut().find(|s| s.id == id) {
            s.zeroized = true;
        }
    }

    pub fn unzeroized_count(&self) -> usize {
        self.tracked_secrets.lock().unwrap().iter().filter(|s| !s.zeroized).count()
    }
    ...
    pub fn audit_report(&self) -> ZeroizationReport {
        ...
        ZeroizationReport { total_secrets: total, zeroized, unzeroized: total - zeroized }
    }
}

impl Default for ZeroizationAuditor { fn default() -> Self { Self::new() } }
...
impl BackupManager {
    pub fn new() -> Self { Self { backups: Mutex::new(HashMap::new()) } }

    pub fn create_backup(&self, backup_id: &str, shares: &[Vec<u8>], threshold: u32) -> EncryptedBackup {
        use rand_core::{OsRng, RngCore};
        let encrypted: Vec<String> = shares.iter().map(|s| {
            let mut key = [0u8; 32];
            OsRng.fill_bytes(&mut key);
            let encrypted: Vec<u8> = s.iter().enumerate()
                .map(|(i, &b)| b ^ key[i % key.len()])
                .collect();
            hex::encode(encrypted)
        }).collect();
        let backup = EncryptedBackup {
            backup_id: backup_id.into(), encrypted_shares_hex: encrypted,
            backup_key_threshold: threshold, created_at: Utc::now(),
        };
        self.backups.lock().unwrap().insert(backup_id.into(), backup.clone());
        backup
    }
    ...
    pub fn restore_backup(&self, backup_id: &str, keys: &[[u8; 32]; 32]) -> Option<Vec<Vec<u8>>> {
        ...
        if keys.len() < backup.backup_key_threshold as usize { return None; }
        // Simplified restore: XOR with keys (mock)
        let restored: Vec<Vec<u8>> = backup.encrypted_shares_hex.iter().map(|hex_str| {
    ...
    pub fn backup_count(&self) -> usize { self.backups.lock().unwrap().len() }
...
impl Default for RetentionPolicy {
    fn default() -> Self {
        Self { session_retention_days: 7, audit_retention_days: 365,
               wal_retention_entries: 100_000, purge_interval_hours: 1 }
    }
}
...
impl RetentionEngine {
    pub fn new(policy: RetentionPolicy) -> Self {
        Self { policy, purged_sessions: AtomicU64::new(0),
    ...
    pub fn purge_sessions(&self, sessions: &mut Vec<(String, DateTime<Utc>)>) -> usize {
        ...
        self.purged_sessions.fetch_add(purged as u64, Ordering::SeqCst);
    ...
    pub fn truncate_wal(&self, wal: &mut Vec<String>) -> usize {
        ...
        if wal.len() <= max { return 0; }
        let excess = wal.len() - max;
        wal.drain(..excess);
        self.purged_audit_entries.fetch_add(excess as u64, Ordering::SeqCst);
        excess
    }

    pub fn total_purged_sessions(&self) -> u64 { self.purged_sessions.load(Ordering::SeqCst) }
    pub fn total_purged_entries(&self) -> u64 { self.purged_audit_entries.load(Ordering::SeqCst) }
}
...
impl ReplicationManager {
    pub fn new() -> Self {
        Self { replicas: Mutex::new(HashMap::new()), local_seq: AtomicU64::new(0) }
    }

    pub fn register_replica(&self, region: &str) {
        self.replicas.lock().unwrap().insert(region.into(), ReplicaState {
            region: region.into(), last_seq: 0, last_sync: Utc::now(), lag_seconds: 0,
        });
    }
    ...
    pub fn advance_local(&self) -> u64 { self.local_seq.fetch_add(1, Ordering::SeqCst) + 1 }
    pub fn local_seq(&self) -> u64 { self.local_seq.load(Ordering::SeqCst) }
    ...
    pub fn replica_count(&self) -> usize { self.replicas.lock().unwrap().len() }
    pub fn all_replicas(&self) -> Vec<ReplicaState> {
        self.replicas.lock().unwrap().values().cloned().collect()
    }
}

impl Default for ReplicationManager { fn default() -> Self { Self::new() } }
...
impl MockKms {
    pub fn new() -> Self { Self { keys: Mutex::new(HashMap::new()) } }
}
...
impl CloudKms for MockKms {
    fn encrypt(&self, plaintext: &[u8], key_id: &str) -> Result<Vec<u8>, String> {
        ...
        Ok(plaintext.iter().enumerate().map(|(i, &b)| b ^ key[i % 32]).collect())
    }
    fn decrypt(&self, ciphertext: &[u8], key_id: &str) -> Result<Vec<u8>, String> {
    ...
    fn name(&self) -> &str { "mock-kms" }
}

impl Default for MockKms { fn default() -> Self { Self::new() } }
...
impl SyslogMessage {
    pub fn new(severity: u8, message: &str) -> Self {
        Self { facility: 4, severity, timestamp: Utc::now(),
               hostname: "confium".into(), app_name: "coordinator".into(),
               proc_id: 1, msg_id: "audit".into(), message: message.into() }
    }

    pub fn priority(&self) -> u8 { self.facility * 8 + self.severity }

    pub fn to_rfc5424(&self) -> String {
        format!("<{}>1 {} {} {} {} {} {}",
            self.priority(),
            self.timestamp.format("%Y-%m-%dT%H:%M:%SZ"),
            self.hostname, self.app_name, self.proc_id, self.msg_id, self.message)
    }
}
...
impl SyslogForwarder {
    pub fn new() -> Self { Self { sent: Mutex::new(Vec::new()), enabled: Mutex::new(true) } }
    ...
    pub fn messages(&self) -> Vec<SyslogMessage> { self.sent.lock().unwrap().clone() }
...
mod tests {
    ...
    fn frequency_test_random_data() {
        ...
        assert!(p_value > 0.01, "p-value should be > 0.01 for random data, got {p_value}");
    }
    ...
    fn entropy_random_is_high() {
        ...
        assert!(entropy > 7.0, "entropy should be > 7.0 bits/byte for random data");
    }
    ...
    fn retention_purges_old_sessions() {
        let engine = RetentionEngine::new(RetentionPolicy { session_retention_days: 1, ..Default::default() });
    ...
    fn retention_truncates_wal() {
        let engine = RetentionEngine::new(RetentionPolicy { wal_retention_entries: 5, ..Default::default() });
