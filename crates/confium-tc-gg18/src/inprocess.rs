...
//!
//! - `shares[i]` is the opaque `Gg18Share::to_bytes()` encoding for
...
/// Outcome of a single GG18 DKG run: N share blobs plus the joint
/// public key.
#[derive(Debug, Clone)]
...
/// Drive the GG18 DKG in-process for `party_count` parties at threshold
/// `threshold`.
///
...
/// Drive a GG18 signing session in-process using `share_blobs` (each a
/// `Gg18Share::to_bytes()` blob from a previous [`keygen`]) at threshold
/// `threshold`. Returns the 64-byte `(r, s)` ECDSA signature.
///
...
mod tests {
    ...
    fn keygen_and_sign_round_trip() {
        ...
        let sig = sign(&kg.shares[..2], 2, b"hello gg18").expect("sign");
        ...
        vk.verify(b"hello gg18", &s).expect("verify ok");
    }
