//! End-to-end integration tests for GG18 threshold ECDSA over P-256.
//!
...
//! 3. Any T-of-N subset produces the same-threshold signature.
...
fn verify_sig(share_bytes: &[u8], msg: &[u8], sig: &[u8]) -> bool {
    let share = Gg18Share::from_bytes(share_bytes).expect("share decodes");
...
fn threshold_property_any_subset_same_key() {
    ...
    // Signatures over the same message with the same key and the same
    // deterministic nonce-reveal path may differ (the nonces are fresh
    // per session), but both must verify.
}
...
fn signing_without_share_aborts() {
    ...
    // SchemeInternalError (0x1041) with GG18 BAD_SHARE sub-code.
...
fn byzantine_partial_signature_aborts() {
    ...
    // corrupted share (off-by-one in the secret), so its partial is
    ...
    let mut carol = Gg18Share::from_bytes(&shares[2]).expect("share");
